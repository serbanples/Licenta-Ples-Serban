...
import { mockCreateElasticLogger, mockError, mockWinstonLogger } from "./logger.mock";
...
describe('LoggerService', () => {
    ...
    describe('initialization', () => {
      ...
      it('should handle elasticsearch transport errors', () => {
        ...
        // Emit the error event after service initialization
        process.nextTick(() => {
        ...
        return new Promise<void>((resolve) => {
          setTimeout(() => {
            expect(consoleSpy).toHaveBeenCalledWith('Elasticsearch transport error:', mockError);
            resolve();
          }, 0);
        });
      });
